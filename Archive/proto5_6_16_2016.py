...
class Beat:
    ...
    def interleave_split(self, times_map, total_time=1.0):
        ...
        portions = [t2[0] - t1[0] for t1, t2 in zip(sequence, sequence[1:] + [(total_time, None)])]
        beats_for_labels = {}
    ...
    def _descendents_duration(self):
        if self.duration
...
class Track:
    ...
    def __str__(self):
        return self.name + " (%.1f-%.1f)" % (self.start_time, self.start_time + self._duration)
