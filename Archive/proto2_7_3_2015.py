...
class RawSound(Sound):
    ...
    def __init__(self, rate, filename, registration_point=0.0):
        self.mono_data = self._read_mono_data(rate, filename)
...
class RandomSound(Sound):
    ...
    def render_from(self, location):
        return self.sounds[np.random.randint(len(self.sounds))].render_from(location)
    ...
    def populate_with_dir(self, dir):
        for file_name in os.listdir(dir):
            ...
            if parse[-1] != 'wav':
                continue
            if len(parse) > 2:
                reg_pt = float(parse[1])/1000 # field after first dot in file name is reg_pt ms
            else:
                reg_pt = 0.0
            self.sounds.append(RawSound(self.rate, os.path.join(dir, file_name), reg_pt))
        return self
...
class ResampledSound(Sound):
    ...
    def render_from(self, location):
        ...
        i = -1
        stop = False
        negative_mark = reg_pt * self.rate
        while not stop:
            interp_points = self._points_for_block(block_size, i)
            interp_points = interp_points - interp_points[-1] + negative_mark # i_p[-1] is the maximum
            ...
            negative_mark = interp_points.min()
            i -= 1
        
        # positive times
        print "interpolated from " + str(i + 1)
        i = 0
        stop = False
        ...
        while not stop:
            ...
            i += 1
        print "to " + str(i - 1)
        return (negative_mark/self.rate, resampled_data)
...
if __name__ == "__main__":
    ...
    hit.populate_with_dir("audio\\TheLight\\G_major_xylo\\A")
    for i, quarter in enumerate(quarters):
        res_hit = ResampledSound(rate, hit, lambda x: .5**(x/2))
        quarter.attach(res_hit, (0, 1))
